...
export class AddEmployeeComponent implements OnInit {
  ...
  onSubmit(){  
    ...
    if (this.employeeForm.valid) {
      ...
      this.add_obj = this.employeeForm.value;
      this.add_obj.entity_id =  this.entity_id;

      if(this.id ==null){
        this.service.apiPost('addEmployee', this.add_obj)
        .subscribe(result => {
              this.result = result
       ...
       this.obj.id = this.id;
        ...
        this.service.apiPost('updateEmployee/'+this.obj.id,this.obj )
        .subscribe(result => {
          this.result = result;
          console.log(this.result)
  ...
  getEmployeeById(id){
    ...
    this.service.apiGet('getEmployeeById/'+id)   
    .subscribe(
      result => {
        this.result = result
        if(this.result.status == "success"){
  ...
  packageCheck(package_id, entity_id){
        ...
        .subscribe(result => {
         ...
         if(this.result.data.guard_length == 0){
          this.toastr.success('please add the guard');
          this.router.navigate(['guard/add-guard']);
         }else {
